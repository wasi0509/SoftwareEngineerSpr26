//
// $Id: firefly.b,v 1.2 2004/08/17 09:16:42 lindig Exp $
// 
// How we use markers:
// 
...
//  5           _ 1 2
//
// Marker 4 is set when a valid vector was written.  Marker 3 is set if
// food can be found following the vector in *opposite* direction.
// Marker 5 is unused
//
...
define find_home {
    -- random walk to find a trail home, or home itself
    loop {
        ...
        flip 8 then turn right;
        flip 8 then turn left;
        move else loop {turn right; flip 2 then exit; }
    }
}

-- follow a vector backwards that has its food bit set 

define follow_food_trail {
    loop {
        sense here food then sense here home else { 
            pickup; 
            goto follow_trail_home; 
        }
        
        -- erase food bit, will be set again on way back with food.
        unmark 3; 
        
        -- turn into *opposite* direction of the vector
        sense here marker 4 else goto find_food;
        sense here marker 0 then {
            sense here marker 1 then {
                sense here marker 2 then {
                    -- illegal marker
        ...
        -- we are now facing away from the vector 
        loop {
            sense ahead food then exit;
            sense left ahead food then {turn left; exit;}
            sense right ahead food then {turn right; exit;}
            sense left ahead marker 3 then {turn left; exit;}
            sense right ahead marker 3 then {turn right; exit;}
            sense ahead marker 3 then exit;
            -- no food at end of food trail
            loop {
                turn right;
                flip 4 then move then goto find_food;
            }    
            exit;        
        }
        -- collision strategy    
        move else choose {
            case: { turn left;  move; goto find_food; }
            case: { turn right; move; goto find_food; }
        }    
    }
}

-- find food, leave a trail of vectors pointing home
...
define find_food {
    loop {
        ...
        sense here marker 3 then flip 8 else goto follow_food_trail;
